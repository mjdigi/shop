...
const APP_LINK = "https://drive.google.com/file/d/12in5ZXueNrQQ3SxA-xm-gEsCzUQr3Lsm/view?usp=sharing";

function initAppDownloadFeatures() {
    // 1. Add Download Icon to Header
    const headerIcons = document.querySelector('.header-icons');
    if (headerIcons) {
        // Create the download button
        const downloadBtn = document.createElement('button');
        downloadBtn.className = 'icon-btn';
        downloadBtn.innerHTML = '<i class="fas fa-download"></i>'; // Using generic download icon
        ...
        // Insert it as the first element in the icons container
        headerIcons.insertBefore(downloadBtn, headerIcons.firstChild);
    }

    // 2. Check and Show Popup (First Time Only)
    // We check localStorage for a specific flag
    const hasSeenPopup = localStorage.getItem('wimhub_app_popup_seen');
    
    if (!hasSeenPopup) {
        // Delay slightly for better UX
        setTimeout(showAppDownloadPopup, 2000);
    }
}
...
function showAppDownloadPopup() {
    ...
    popup.style.cssText = `
        background: white; width: 85%; max-width: 320px; padding: 2rem;
        border-radius: 20px; text-align: center; box-shadow: 0 20px 50px rgba(0,0,0,0.3);
        transform: scale(0.8); transition: transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);
    `;

    popup.innerHTML = `
        <div style="font-size: 3.5rem; color: #054a29; margin-bottom: 1rem;"><i class="fab fa-android"></i></div>
        <h3 style="margin-bottom: 0.5rem; font-family: 'Playfair Display', serif; font-size: 1.5rem;">Download App</h3>
        <p style="color: #666; margin-bottom: 1.5rem; font-size: 0.95rem; line-height: 1.5;">
            Get the full WimHub experience! Download our Android App for faster checkout and exclusive deals.
        </p>
        <button id="popup-dl-btn" style="
            background: #054a29; color: white; border: none; padding: 12px 24px;
            width: 100%; border-radius: 12px; font-weight: 600; margin-bottom: 12px; 
            cursor: pointer; font-size: 1rem; box-shadow: 0 4px 12px rgba(5,74,41,0.2);
        ">Download Now</button>
    ...
    // Event Handlers
    document.getElementById('popup-dl-btn').addEventListener('click', () => {
        window.open(APP_LINK, '_blank');
        closePopup();
    });
    ...
    function closePopup() {
        ...
        localStorage.setItem('wimhub_app_popup_seen', 'true');
    }
}
...
async function initHome() {
    ...
    // Initialize the new download features
    initAppDownloadFeatures();
}
